...
def checkExcersice(query_set):
    ...
    if np.all(real_data[:,1:-1] == data):
     # correct data, I'm getting further
        ...
        cond_p2 = np.round(P, 2) == np.round(P_real, 8)*10**6
        ...
        # If some of them are not good I need to stop evaluation here
            ...
            if not Hp_condition:
                if np.all(HP == np.zeros(np.shape(HP))):
                    comments.append(f"Współrzędne przybliżone nie zostały wprowadzone")
                else:
        ...
        comment = f'Zostały wzięte niewłaściwe dane, dalsze sprawdzanie mija się z celem'
